...
export function setup() {
  ...
  if (res.status === 200) {
    ...
    return {
      seedAccountID: body.accountId || null,
      seedAccountNumber: body.accountNumber || null,
    };
  }
...
export default function (data) {
  ...
  group("GET /accounts/{id}", () => {
    ...
    const res = http.get(`${BASE_URL}/accounts/${vuAccountID}`, {
      headers: snapHeaders(uuidv4()),
...
export function teardown(data) {
  console.log(
    `[account-service] done. seed: ${data.seedAccountNumber || "N/A"}`,
  );
}
